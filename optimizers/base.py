...
class Optimizer():
    ...
    def optimize(self, x0, verbose=False):
        ...
        if self.verbose:
            ...
            print_row('0', '%3.3f' % info[0], '%3.3f' % info[1], 'nan', 'nan', '%3.3f' % self.fx, '%s' % str(self.x.reshape(-1)))
        while True:
            start = time.time()
            ...
            if self.verbose and not (reason is not None and "constraints violated" in reason):
                print_row('%d' % self.it, '%3.3f' % info[0], '%3.3f' % info[1], '%3.3f' % self.xdiff, '%3.3f' % self.fdiff, '%3.3f' % self.fx_next, '%s' % str(self.x_next.reshape(-1)))
            if finish:
        ...
        if self.verbose:
            ...
            print_row('%3.3f' % info[0], '%3.3f' % info[1], '%3.3f' % self.fx_next, '%s' % str(self.x_next.reshape(-1)), width=[16, 16, 16, 40])
            print('-' * 99)
        return self.track
